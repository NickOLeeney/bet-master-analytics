...
def objective(trial, min_obs:int, _lambda: float, feature_bins_map: dict, df_binned: pd.DataFrame, suggest_feature: bool = True, objective_type: str = "robust_risk_adjusted_score"):
    ...
    all_weeks = df_binned["week"].dropna().unique()
    ...
    mean_weekly_return = weekly_returns.mean() # massimizzo ritorno settimanale
    std_weekly_return = weekly_returns.std(ddof=0) # minimizzo volatilità ritorno settimanale

    if objective_type == "robust_risk_adjusted_score":
        downside_dev = np.sqrt(
            np.mean(np.minimum(weekly_returns, 0) ** 2)
        )

        alpha = 0.10
        q = weekly_returns.quantile(alpha)

        cvar_return = weekly_returns[
        weekly_returns <= q
        ].mean()

        cvar_loss = max(0, -cvar_return)

        standard_error = (
        std_weekly_return / np.sqrt(len(weekly_returns))
        )
    ...
    elif objective_type == "mean_variance_score":
